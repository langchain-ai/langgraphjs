...
It includes five agent targets exposed through `langgraph.json`:
...
- `parallel-subagents`: a focused Deep Agent fan-out demo for namespace-aware protocol UIs
- `human-in-the-loop`: a `createAgent(...)` workflow that pauses on a reviewed tool call
