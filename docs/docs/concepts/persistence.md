...
- `waitingEdges`: Present only when a waiting edge — one created by `addEdge([...], target)` — holds writes from some of its listed nodes but not all, so it has not released. Each entry names the `target`, the nodes that `completed` and the nodes still `missing`. Alongside an empty `next` it means the run ended without releasing the edge, so those writes were discarded. An entry does not mean `target` never ran: it can also be reached by an earlier pass of a loop or by a `Send`, so read it as "these writes were dropped" rather than "this node was never executed". Entries inside subgraphs are included when `getState` is called with `subgraphs: true`, each carrying the subgraph node `path` it belongs to. An edge created with `addEdge([...], target, { inclusive: true })` never appears here alongside an empty `next`: it always fires — at latest when the resumed run settles — so its target is reported in `next` whenever nothing else is scheduled.
