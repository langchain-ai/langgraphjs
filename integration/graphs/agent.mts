...
// For shared state (store-based, replaces removed SharedValue)
const sharedValueNs = ["sharedValue"];
...
const agentNode = async (
  ...
  let sharedStateValue: string | null = null;
  if (store && userId) {
    const existing = await store.get(sharedValueNs, userId);
    sharedStateValue = existing?.value?.user_id ?? null;
    await store.put(sharedValueNs, userId, { user_id: userId });
  }
...
export const graph = async (config: LangGraphRunnableConfig) => {
  const delay = config.configurable?.delay ?? 0;
