...
`injectStream` can talk to LangGraph Platform directly, or to anything
...
- Custom A2A events are normalized server-side into the protocol's
  `custom:<name>` convention and read with `injectExtension`.

The example UI is React, but the transport and server shape are
framework-agnostic. Angular apps use the same adapter and the same
server contract.
...
## Client Setup In Angular

Create the adapter once and pass it to `injectStream` or
`provideStream`. Use `paths` when your server routes live under an
app-specific prefix, as in the example.
...
To share one stream across a subtree, provide the adapter once:
...
Children call `injectStream()` with no arguments. Keep adapter
construction in a field initializer, provider, or other one-time
...
In an Angular SSR or NestJS app, the same shape maps to server routes
that read the command or subscription body and delegate to your
session layer.
...
requested channels, namespaces, and depth. This lets selectors mount
after a run has already started without forcing the graph to restart.
...
After normalization, `injectExtension(stream, "a2a")` subscribes to
`custom:a2a` and receives `params.data.payload`.

```typescript
import { Component, effect, signal } from "@angular/core";
import { injectExtension, injectStream } from "@langchain/angular";

@Component({
  standalone: true,
  selector: "app-a2a-panel",
  template: `
    @for (event of events(); track $index) {
      <pre>{{ asJson(event) }}</pre>
    }
  `,
})
export class A2APanelComponent {
  readonly stream = injectStream<GraphType>();
  readonly a2a = injectExtension<A2AStreamEvent>(this.stream, "a2a");
  readonly events = signal<A2AStreamEvent[]>([]);

  constructor() {
    effect(() => {
      const payload = this.a2a();
      if (payload != null) this.events.update((cur) => [...cur, payload]);
    });
  }
...
- **Using `custom` instead of `custom:<name>`.** `custom` receives all
  custom events; `injectExtension(stream, "a2a")` subscribes to
  `custom:a2a`.
...
- [Selectors and `injectExtension`](./selectors.md)
