...
that implements the Agent Streaming Protocol. The key question is
whether you need a **custom browser transport**, or just a **custom
backend**.

The [`examples/ui-react-transport`](../../../examples/ui-react-transport)
example demonstrates the recommended custom-backend path:

- The React app uses the stock `HttpAgentServerAdapter`.
...
That means you can bring your own server without making every app ship
and maintain a custom `AgentServerAdapter`.
...
The example creates a stock adapter with custom route paths:
...
thread and exposes two protocol endpoints:
...
- [`StreamProvider` / `useStreamContext`](./context.md)
- [Selectors and `useExtension`](./selectors.md)
