...
const toNamespacedEvent = (event: StreamEvent, namespace: string[]) =>
  namespace.length > 0 ? (`${event}|${namespace.join("|")}` as StreamEvent) : event;
...
const toMessageMetadata = (
  ...
  const tags = Array.isArray(metadata?.tags)
    ...
    : [];
  const checkpointNs =
    namespace.length > 0 ? namespace.join("|") : undefined;
...
export function getProtocolChannels(streamMode?: StreamMode | StreamMode[]) {
  ...
  const channels = new Set<ProtocolChannel>(["lifecycle"]);
...
export class ProtocolEventAdapter {
  ...
  private toLegacyMessageEvents(
    ...
    const state =
      activeMessageId != null ? this.messageState.get(activeMessageId) : undefined;
    ...
    switch (data.event) {
      ...
      case "content-block-finish":
        if (state == null || typeof data.index !== "number" || !isRecord(data.contentBlock)) {
          return [];
        }
  ...
  private toLegacyContentBlockEvent(
    ...
    phase: "content-block-start" | "content-block-delta" | "content-block-finish",
    index: number,
    ...
    if (blockType === "text") {
      ...
      return [
        this.createMessageTupleEvent(
          event.eventId,
          namespace,
          createAiChunk(state.id, { content: delta }),
          metadata
        ),
      ];
    }
    ...
    if (blockType === "reasoning") {
      const nextReasoning =
        typeof contentBlock.reasoning === "string" ? contentBlock.reasoning : "";
    ...
    if (blockType === "tool_call_chunk") {
      ...
      const mergedArgs =
        phase === "content-block-delta" ? `${previousArgs}${argsText}` : argsText;
      const toolId =
