 ...
 *
 * Validates that:
 * - stream.messages is BaseMessage[] class instances
 * - stream.toolCalls is correctly typed from deep agent tools
 * - stream.values contains the expected agent state
 * - stream.subagents contains the right discovery snapshot types
...
describe("deep agent", () => {
  test("has well typed messages", () => {
    ...
    if (AIMessage.isInstance(stream.messages.value[0])) {
      expectTypeOf(stream.messages.value[0].tool_calls).toExtend<
        | {
    ...
    if (ToolMessage.isInstance(stream.messages.value[0])) {
      expectTypeOf(stream.messages.value[0].content).toEqualTypeOf<
  ...
  test("has well typed values", () => {
    ...
    expectTypeOf(stream.values.value.todos).toEqualTypeOf<
      {
        content: string;
    ...
    expectTypeOf(stream.values.value.files).toEqualTypeOf<
      {
        path: string;
        content: string;
      }[]
    >();
    expectTypeOf(stream.values.value.notes).toEqualTypeOf<
      {
        title: string;
        body: string;
        createdAt: string;
  ...
  test("should have well typed subagent values", () => {
    ...
    const subagent = [...stream.subagents.value.values()][0];
