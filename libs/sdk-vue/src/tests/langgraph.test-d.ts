 ...
 *
 * Validates that:
 * - stream.messages is BaseMessage[] (Vue-specific class instances)
...
describe("graph: stream.values has correct state type", () => {
  ...
  test("research graph: values has all custom fields", () => {
    ...
    expectTypeOf(
      stream.values.value.analyticalResearch,
  ...
  test("pipeline graph: values has nested object and array fields", () => {
    ...
    expectTypeOf(stream.values.value.parsedData.entities).toEqualTypeOf<
      string[]
    >();
    ...
    >();
    expectTypeOf(
      stream.values.value.parsedData.confidence,
