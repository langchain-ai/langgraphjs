 ...
 *
 * Live state is exposed as Vue refs so templates auto-unwrap; all
 * imperative controls are plain functions.
 */
export interface AudioPlayerHandle {
  readonly status: Readonly<Ref<PlayerStatus>>;
  readonly strategy: Readonly<Ref<"pcm" | "element">>;
  play(): void;
  pause(): void;
  stop(): void;
  toggle(): void;
...
export function useAudioPlayer(
  ...
  const level = ref(0);
  const strategyRef = ref<"pcm" | "element">("element");

  // ── Shared refs (mutable state not exposed to templates) ────────────
  ...
  const tickAnalyser = () => {
    ...
    if (ctx != null && status.value === "playing") {
      currentTime.value = ctx.currentTime - playStartCtxTime;
    }
  ...
  const bindElement = (m: AudioMedia): (() => void) => {
    ...
    m.objectURL.then(
      (resolved) => {
        ...
        const onError = () => {
          error.value = new Error("HTMLAudioElement error");
          status.value = "error";
        };
