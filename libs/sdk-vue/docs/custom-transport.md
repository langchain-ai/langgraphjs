...
framework-agnostic. Vue apps use the same adapter and the same server
contract.
...
In a Nuxt / Nitro app, the same shape maps to server routes that read
the command or subscription body and delegate to your session layer.
...
import { useExtension, useStreamContext } from "@langchain/vue";

const stream = useStreamContext<GraphType>();
const events = ref<A2AStreamEvent[]>([]);
const a2a = useExtension<A2AStreamEvent>(stream, "a2a");

watch(a2a, (payload) => {
  if (payload != null) events.value = [...events.value, payload];
});
</script>
```
...
- [Sharing a stream](./sharing-streams.md)
