 ...
 *
 * Collision errors use a branded template literal that names the channel
 * (`Node name "…" is already a state channel name`) instead of `never`.
 */
...
describe("addNode channel-name collisions", () => {
  describe("literal collisions on state keys", () => {
    it("type-errors and throws for an Annotation state key (issue #2724)", () => {
      ...
      expect(() => {
        // @ts-expect-error Node name "judge" is already a state channel name
    ...
    it("type-errors addNode(key, action, { ends }) on a colliding channel", () => {
      const State = Annotation.Root({ judge: Annotation<string>() });
      const graph = new StateGraph(State);

      expect(() => {
        // Fixed-length error tuple: a rest `unknown[]` slot would let
        // this compile. Inline `{ ends }` so K stays the colliding
        // literal and the branded message still names "judge".
        // @ts-expect-error Node name "judge" is already a state channel name
        graph.addNode("judge", (state) => ({ judge: state.judge }), {
          ends: [END],
        });
  ...
  describe("input / output schema keys (the incoming trap)", () => {
    it("type-errors for an input-only key that is not in state", () => {
      ...
      expect(() => {
        // @ts-expect-error Node name "incoming" is already a state channel name
    ...
    it("type-errors for an output-only key that is not in state", () => {
      ...
      expect(() => {
        // @ts-expect-error Node name "outgoing" is already a state channel name
  ...
  describe("legal literals", () => {
    ...
    it("infers legal node names for addEdge after addNode", () => {
      const State = Annotation.Root({ judge: Annotation<string>() });
      const builder = new StateGraph(State).addNode(
        "process",
        (state) => ({ judge: state.judge }),
        { ends: [END] }
      );

      // Registered node name stays assignable; an unknown name does not.
      builder.addEdge(START, "process");
      builder.addEdge("process", END);
      // @ts-expect-error "missing" is not a registered node
      builder.addEdge("missing", END);
    });
  ...
  describe("wide keys stay allowed", () => {
    ...
    it("type-errors a known channel key on the deprecated { channels } constructor", () => {
      ...
      expect(() => {
        // @ts-expect-error Node name "count" is already a state channel name
