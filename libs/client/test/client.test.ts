...
describe("ProtocolClient", () => {
  ...
  it("forwards object-shaped helper params to protocol commands", async () => {
    const transport = new MockTransport({
      capabilities: {
        modules: [
          ...
          {
            name: "resource",
            commands: ["list", "read", "write", "download"],
            channels: ["resource"],
          },
          {
            name: "input",
            commands: ["respond", "inject"],
            channels: ["input"],
          },
          {
            name: "state",
            commands: ["get", "storeSearch", "storePut", "listCheckpoints", "fork"],
            channels: ["state"],
          },
        ],
      },
    ...
    await session.state!.storeSearch({
    ...
    expect(transport.sentCommands.slice(-5).map((command) => command.method)).toEqual([
      "resource.list",
      "input.inject",
      "input.respond",
      "state.storeSearch",
    ...
    expect(transport.sentCommands.at(-5)?.params).toEqual({
      namespace: ["agent_1"],
      path: "/workspace/src",
    });
    expect(transport.sentCommands.at(-4)?.params).toEqual({
      namespace: ["agent_1"],
      message: {
        role: "user",
        content: "Also check the error handling",
    ...
    expect(transport.sentCommands.at(-3)?.params).toEqual({
      namespace: ["agent_1"],
      interruptId: "interrupt_1",
      response: { approved: true },
    });
    expect(transport.sentCommands.at(-2)?.params).toEqual({
      storeNamespace: ["user_123", "memories"],
      query: "quantum computing",
    });
    expect(transport.sentCommands.at(-1)?.params).toEqual({
      checkpointId: "checkpoint_abc",
      input: { messages: [{ role: "user", content: "Retry" }] },
    });
  });
