...
beforeAll(async () => {
  ...
  await store.setup();
  await store.setup();
  for (const index of ["store", "store_vectors"]) {
    await vi.waitFor(
      async () => {
        const info = await container.client.sendCommand(["FT.INFO", index]);
        expect(
          Array.isArray(info) && Number(info[info.indexOf("indexing") + 1])
        ).toBe(0);
      },
      { timeout: 30000 }
    );
...
it.each([false, true])("isolates segments with vector=%s", async (vector) => {
  for (const namespace of namespaces) {
    const key = `key${namespaces.indexOf(namespace)}`;
    expect((await store.get(namespace, key))?.key).toBe(key);
