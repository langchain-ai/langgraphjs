...
describe("ensureLangGraphConfig", () => {
  ...
  it("should return a default config when no arguments provided", () => {
    ...
    AsyncLocalStorageProviderSingleton.getRunnableConfig = jest.fn().mockReturnValue(undefined);
  ...
  it("should copy values from AsyncLocalStorage if available", () => {
    ...
    AsyncLocalStorageProviderSingleton.getRunnableConfig = jest.fn().mockReturnValue(asyncLocalStorageConfig);
    
    const result = ensureLangGraphConfig();
    
    expect(result.tags).toEqual(["storage-tag"]);
    expect(result.metadata).toEqual({ storage: "value", storageOption: "value" });
...
describe("getStore, getWriter, getConfig", () => {
  ...
  it("getStore should return store from config", () => {
    ...
    AsyncLocalStorageProviderSingleton.getRunnableConfig = jest.fn().mockReturnValue({
      store: mockStore
    });
  ...
  it("getConfig should return the full config", () => {
    ...
    AsyncLocalStorageProviderSingleton.getRunnableConfig = jest.fn().mockReturnValue(mockConfig);
