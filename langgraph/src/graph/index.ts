...
> {
  ...
  addConditionalEdges(
    ...
    if (condition.constructor.name === "AsyncFunction") {
      throw new Error("Condition cannot be a coroutine function");
    }
  ...
  compile(): Pregel {
    ...
    const allStarts = new Set(
      [...this.edges].map(([src]) => src).concat(Object.keys(this.branches))
    );
    ...
    for (const key of Object.keys(this.nodes)) {
      ...
      if (outgoing) {
        nodes[edgesKey] = (
          nodes[edgesKey] as ChannelInvoke<RunInput, RunOutput>
        ).pipe(Channel.writeTo(...outgoing));
      }
